...
/*void    init_table(t_table *table, int nb_philos, int time_to_die, int time_to_eat, int time_to_sleep, int nb_times)
{
    int     i;

    table->number_of_philosophers = nb_philos;
    table->time_to_die = time_to_die;
    table->time_to_eat = time_to_eat;
    table->time_to_sleep = time_to_sleep;
    table->nb_times_each_must_eat = nb_times;
    table->is_dead = 0;
    table->is_full = 0;

    // Allocate memory for philosophers
    table->philos = (t_philo *)malloc(sizeof(t_philo) * nb_philos);
    if (table->philos == NULL)
        exit(EXIT_FAILURE);

    // Initialize philosophers and forks (original code)

    // Create a monitoring thread (detached)
    pthread_create(&table->monitor_thread, NULL, &begin_monitoring, table);
    pthread_detach(table->monitor_thread); // Detach the monitoring thread
}*/
/*
